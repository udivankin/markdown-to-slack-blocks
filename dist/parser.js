        ...
        if (node.type === "heading") {
            ...
            if (node.children.length === 1 && node.children[0].type === "image") {
                ...
                if (options.preferSectionBlocks !== false) {
                    ...
                    currentRichTextElements.push({
                        type: "rich_text_section",
                        elements: node.children.flatMap((child) => isInlineNode(child) ? mapInlineNode(child, options) : []),
                    });
                }
            }
        }
            ...
            const processList = (listNode, indent) => {
                ...
                for (const listItem of listNode.children) {
                    ...
                    const paragraphElements = listItem.children.flatMap((child) => {
                        if (!isParagraphNode(child))
                            return [];
                        return child.children.flatMap((c) => isInlineNode(c) ? mapInlineNode(c, options) : []);
                    });
            ...
            flushRichText();
            const cellElements = node.children.map((row) => row.children.map((cell) => cell.children.flatMap((c) => isInlineNode(c) ? mapInlineNode(c, options) : [])));
            if (options.tableBlockType === "table") {
                const rows = cellElements.map((row) => row.map((elements) => ({
                    type: "rich_text",
                ...
                const rows = cellElements.map((row) => row.map((elements) => buildDataTableCell(elements)));
 ...
 */
function buildDataTableCell(elements) {
    if (elements.length === 0) {
        return { type: "raw_text", text: "" };
    }
    if (elements.length === 1) {
        const [element] = elements;
        const isPlainText = element.type === "text" &&
...
function flattenStyles(children, style, options) {
    const elements = children.flatMap((c) => isInlineNode(c) ? mapInlineNode(c, options) : []);
    ...
    while (match !== null) {
        ...
        if (match[1]) {
            ...
            // 3. Check User Groups
                ...
                elements.push(withStyle({
                    type: "usergroup",
                    usergroup_id: options.mentions.userGroups[name],
                }));
                mapped = true;
            }
            // 4. Check Teams
            ...
            if (options.mentions?.channels?.[name]) {
                elements.push(withStyle({
                    type: "channel",
                    channel_id: options.mentions.channels[name],
                }));
                mapped = true;
            }
            if (!mapped) {
 ...
 */
function inlineNodesToMrkdwn(nodes, options) {
    return nodes
        .map((node) => isInlineNode(node) ? inlineNodeToMrkdwn(node, options) : "")
        .join("");
}
function inlineNodeToMrkdwn(node, options) {
    if (node.type === "text") {
 ...
 */
function convertTextToMrkdwn(text, options) {
    // Regex to find:
    // 1. Broadcast: <!here> | <!channel> | <!everyone>
    // 2. Mention: <@ID>
    // 3. Channel: <#ID>
    ...
    while (match !== null) {
        ...
        if (match[1]) {
            ...
            // Color: #Hex - leave as-is (not a channel)
            result += fullMatch;
        }
        else if (match[6]) {
            // Channel: <#ID>
