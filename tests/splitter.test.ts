...
describe("splitBlocks", () => {
	...
	describe("basic functionality", () => {
		...
		it("returns blocks unchanged when exactly at block limit", () => {
			const blocks: Block[] = Array(40)
				.fill(null)
				.map((_, i) => createRichTextBlock(`Block ${i}`));
			const result = splitBlocks(blocks);
			expect(result).toHaveLength(1);
	...
	describe("character limit splitting", () => {
		it("splits when exceeding character limit", () => {
			...
			const blocks: Block[] = Array(10)
				.fill(null)
				.map(() => createRichTextBlock(longText));
			const result = splitBlocks(blocks);
			expect(result.length).toBeGreaterThan(1);
	...
	describe("edge cases", () => {
		it("handles single oversized non-rich_text block", () => {
			...
			expect(result.length).toBeGreaterThanOrEqual(1);
			expect(
				result.some((batch) => batch.some((b) => b.type === "table")),
			).toBe(true);
		});
	});
