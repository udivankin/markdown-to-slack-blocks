...
[![npm version](https://img.shields.io/npm/v/markdown-to-slack-blocks?logo=npm&color=cb3837)](https://www.npmjs.com/package/markdown-to-slack-blocks)
[![npm downloads](https://img.shields.io/npm/dm/markdown-to-slack-blocks?logo=npm&color=cb3837)](https://www.npmjs.com/package/markdown-to-slack-blocks)
[![CI](https://github.com/udivankin/markdown-to-slack-blocks/actions/workflows/ci.yml/badge.svg)](https://github.com/udivankin/markdown-to-slack-blocks/actions/workflows/ci.yml)
[![TypeScript](https://img.shields.io/npm/types/markdown-to-slack-blocks?logo=typescript&logoColor=white&color=3178c6)](https://www.typescriptlang.org/)
[![Minzipped size](https://img.shields.io/bundlephobia/minzip/markdown-to-slack-blocks?color=44cc11)](https://bundlephobia.com/package/markdown-to-slack-blocks)
[![Node.js](https://img.shields.io/node/v/markdown-to-slack-blocks?logo=node.js&logoColor=white&color=339933)](https://nodejs.org/)
[![License: MIT](https://img.shields.io/npm/l/markdown-to-slack-blocks?color=blue)](./LICENSE)

A powerful library to convert Markdown text into Slack's Block Kit JSON format (and back).

Turn Markdown — including content generated by LLMs — into ready-to-post Slack [Block Kit](https://api.slack.com/block-kit) messages, with full support for rich text, code blocks, tables, mentions, and more. The conversion is bidirectional: render Slack blocks back to Markdown or plain text whenever you need a fallback.
...
| `data_table` | Slack's native data table (default for Markdown tables); cell types are chosen automatically per cell |
| `table` | Legacy table block (opt-in via `tableBlockType: "table"`); every cell is a `rich_text` block |
...
- **Tables**: GFM tables become Slack's native `data_table` block, with each cell typed dynamically (`raw_text`, `raw_number`, or `rich_text`).
- **Bidirectional**: Convert Markdown → Block Kit and Block Kit → Markdown/plain text for fallbacks.
- **Slack-Specific Extensions**: Support for user mentions, channel mentions, user groups, and team mentions.
- **LLM-Ready**: Designed to clean up Markdown returned by LLMs into valid Block Kit JSON.
- **Configurable**: Options to customize behavior, such as color detection and table block type.
...
Markdown (GFM) tables are converted to Slack's native [`data_table`](https://api.slack.com/reference/block-kit/blocks#table) block by default. The cell type for every cell is chosen **dynamically, based on its content**, so the output is as faithful and lightweight as possible:

| Cell content | Emitted cell type |
|--------------|-------------------|
| Plain, unstyled text | `raw_text` |
| A pure number (e.g. `10`, `-3.5`) | `raw_number` (keeps `value` for sorting/formatting) |
| Styled text, links, mentions, emoji, … | `rich_text` |
...
markdownToBlocks(markdown, {
  tableBlockType: "data_table", // default; use "table" for the legacy rich_text-cell block
  tableCaption: "Data table",   // default caption; pass "" to omit it
});
```

`blocksToMarkdown` and `blocksToPlainText` render both `data_table` and legacy `table` blocks back to Markdown tables, so tables round-trip cleanly.
