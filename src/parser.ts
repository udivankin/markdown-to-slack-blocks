...
function processTextNode(
	...
	const regex =
		/(<!here>|<!channel>|<!everyone>)|(<@([\w.-]+)>)|(#[0-9a-fA-F]{6})|(<#([\w.-]+)>)|(<!subteam\^([\w.-]+)>)|(<!date\^(\d+)\^([^|]+)\|([^>]+)>)|(:([\w+-]+):)|(@([\w.-]+))|(#([\w.-]+))/g;
	...
	while (match !== null) {
		...
		if (match[1]) {
			// Broadcast: <!here>
			const range = match[1].substring(2, match[1].length - 1) as
				| "here"
				| "channel"
				| "everyone";
			...
			if (subteamId.startsWith("S")) {
				elements.push(
					withStyle({ type: "usergroup", usergroup_id: subteamId }),
				);
			} else {
			...
			if (["here", "channel", "everyone"].includes(name)) {
				elements.push(
					withStyle({
						type: "broadcast",
						range: name as "here" | "channel" | "everyone",
					}),
				);
				mapped = true;
			}
