 ...
 */
function renderRichTextBlockBodyAsMarkdown(
	block: RichTextBlock,
	options?: BlocksToMarkdownOptions,
): string {
...
function renderDataTableBlockAsMarkdown(
	block: Extract<Block, { type: "data_table" }>,
	options?: BlocksToMarkdownOptions,
): string {
	if (block.rows.length === 0 || block.rows[0].length === 0) {
		return "";
	...
	const rows = block.rows.map(
		(row) =>
			`| ${row.map((cell) => renderDataTableCellAsMarkdown(cell, options)).join(" | ")} |`,
	);
	const separator = `| ${block.rows[0].map(() => "---").join(" | ")} |`;
...
function renderDataTableCellAsMarkdown(
	cell: DataTableCell,
	options?: BlocksToMarkdownOptions,
): string {
	const rendered =
		cell.type === "raw_text" || cell.type === "raw_number"
			? cell.text
